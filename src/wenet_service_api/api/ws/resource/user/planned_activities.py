...
class WeNetUserPlannedActivitiesInterface(CommonWeNetUserInterface):
    ...
    def get(self, profile_id: str):
        ...
        if isinstance(authentication_result, ComponentAuthentication):
            ...
            if self._is_owner(authentication_result, profile_id):  # and authentication_result.has_scope(Scope.PLANNED_ACTIVITIES):  # TODO check for the reading scope when will be added
                return profile.planned_activities, 200
            else:
                abort(403)
                return
    ...
    def put(self, profile_id: str):
        ...
        try:
            posted_planned_activities: list = request.get_json()
        except Exception as e:
        ...
        try:
            if not isinstance(authentication_result, Oauth2Result):
                ...
                # if authentication_result.has_scope(Scope.PLANNED_ACTIVITIES):  # TODO check for the writing scope when will be added
                updated_profile = self._service_connector_collector.profile_manager_collector.patch_user_profile(patched_profile)
                # else:
                #     abort(403)
                #     return
            ...
            logger.exception(f"Unauthorized to update the planned activities of the profile [{profile_id}]", exc_info=e)
            abort(403)
            return
        # except NotFound as e:
